...
function ScoreRing({ score }) {
  const color = score >= 75 ? "text-green-600" : score >= 55 ? "text-amber-600" : "text-slate-500";
...
export default function Compatibility() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl flex-1 px-4 py-10">
      <h1 className="text-2xl font-bold text-slate-800">🧩 Life Compatibility Score</h1>
          ...
          {COMPONENTS.map((c) => (
            <label key={c.key} className="block">
              <span className="flex justify-between text-sm">
                ...
                <span className="font-bold text-indigo-600">{weights[c.key]}/10</span>
              </span>
      ...
      {data && (
        <div className="mt-6 space-y-3">
          ...
          {data.results.map((r, i) => (
            <div key={r.city} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-1 text-sm text-indigo-700">{r.verdict}</p>
