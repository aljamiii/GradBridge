...
export default function CostPredictor() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 px-4 py-8 sm:px-6 sm:py-10">
      <h1 className="animate-rise text-2xl font-bold tracking-tight text-ink-900 sm:text-[1.75rem]">AI Cost Predictor</h1>
      ...
      {result && (
        <div className="mt-6 rounded-2xl border border-slate-200/80 bg-white p-6 shadow-[var(--shadow-card)]">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            ...
            {result.cached && (
              <span className="text-xs text-ink-400">⚡ served from cache</span>
            )}
          </div>
            ...
            <tbody>
              {rows.map(([label, amount]) => (
                <tr key={label} className="border-b border-slate-100">
                  ...
                  <td className="py-2 text-right font-medium text-ink-900">{usd(amount)}</td>
                </tr>
              ))}
              <tr>
